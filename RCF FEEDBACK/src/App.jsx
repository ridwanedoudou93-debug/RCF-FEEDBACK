...
function App() {
  ...
  if (submitted) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-8 sm:p-10 text-center">
          ...
          <h2 className="text-3xl font-black text-gray-900 mb-4">شكرًا لك!</h2>
          <p className="text-gray-600 mb-8 text-lg">تم استقبال ملاحظاتك بنجاح. نقدر وقتك وثقتك بنا.</p>
          <button
            onClick={() => {
              setSubmitted(false)
              setCurrentStep(0)
            ...
            className="px-10 py-4 bg-gradient-to-r from-[#f47e1b] to-orange-500 text-white rounded-2xl font-bold text-lg hover:opacity-90 transition-all transform hover:scale-105 shadow-lg"
          >
            إرسال ملاحظة أخرى
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 py-8 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
          <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-8 sm:p-10 text-center">
            <div className="w-36 h-36 bg-white rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl border-4 border-white/20">
          ...
          {error && (
            <div className="mx-8 sm:mx-10 mt-6 p-5 bg-red-50 border-2 border-red-200 rounded-2xl text-red-700 font-semibold">
              {error}
            </div>
          )}

          <form onSubmit={currentStep === steps.length - 1 ? handleSubmit : (e) => e.preventDefault()} className="p-8 sm:p-10">
            <div className="space-y-6 sm:space-y-8">
              {steps[currentStep].questions.map((qId, index) => {
                if (qId === 'q11') {
                  return (
                    <div key={qId} className="bg-gradient-to-br from-slate-50 to-white border-2 border-slate-200 rounded-2xl p-6 sm:p-7 shadow-sm hover:shadow-md transition-shadow">
                      <h3 className="text-xl font-bold text-slate-900 mb-5 flex items-center">
                        <span className="w-12 h-12 bg-gradient-to-br from-[#f47e1b] to-orange-500 text-white rounded-2xl flex items-center justify-center ml-4 font-black text-xl shadow-md">
                          11
                        </span>
                        هل لديك اقتراحات أو ملاحظات تساعدنا على التطور؟
                      </h3>
                      <textarea
                        name="q11"
                        value={formData.q11}
                        ...
                        rows="5"
                        className="w-full p-5 border-3 border-slate-200 rounded-xl focus:border-[#35a3fc] focus:outline-none resize-none text-lg font-medium transition-all"
                  ...
                  <div key={qId} className="bg-gradient-to-br from-slate-50 to-white border-2 border-slate-200 rounded-2xl p-6 sm:p-7 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      {q.options.map((option) => (
                        <label 
                          key={option} 
                          className={`flex items-center p-4 sm:p-5 rounded-xl border-3 cursor-pointer transition-all transform hover:scale-[1.02] ${
                            formData[qId] === option 
                        ...
                        >
                          <input
                            ...
                            onChange={handleChange}
                            className="w-6 h-6 text-[#f47e1b] focus:ring-[#f47e1b] focus:ring-offset-2"
                            required
                          />
                          <span className="mr-4 sm:mr-5 text-lg font-semibold text-slate-800">{option}</span>
                        </label>
                      ))}
                    </div>
                  </div>
            ...
            <div className="flex gap-4 mt-10">
              {currentStep > 0 && (
                <button
                  type="button"
                  onClick={handlePrev}
                  className="flex-1 py-4 bg-slate-200 text-slate-800 rounded-2xl font-bold text-lg hover:bg-slate-300 transition-colors"
                >
                  السابق
                </button>
              )}
              {currentStep < steps.length - 1 ? (
                <button
                  ...
                  className={`flex-1 py-4 bg-gradient-to-r from-[#f47e1b] to-orange-500 text-white rounded-2xl font-bold text-lg transition-all transform hover:scale-[1.02] shadow-lg ${!isStepValid() ? 'opacity-50 cursor-not-allowed hover:scale-100' : 'hover:opacity-90'}`}
                >
                  التالي
                </button>
              ) : (
                <button
                  type="submit"
                  disabled={loading}
                  className={`flex-1 py-4 bg-gradient-to-r from-[#f47e1b] via-orange-500 to-[#f47e1b] text-white rounded-2xl font-black text-xl hover:opacity-90 transition-all transform hover:scale-[1.02] shadow-2xl disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none`}
                >
                  {loading ? (
                    <span className="flex items-center justify-center">
                      <svg className="animate-spin h-7 w-7 ml-3" viewBox="0 0 24 24">
